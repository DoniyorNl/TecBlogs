...
import { PopCategories, popularTags } from '@/constants/const'
import { getSearchedBlog } from '@/service/blog.service'
...
function GlobalSearch() {
	...
	const handleSearch = async (e: ChangeEvent<HTMLInputElement>) => {
		...
		if (text && text.length > 2) {
			setIsLoading(true)
			const data = await getSearchedBlog(text)
			setBlogs(data ?? [])
			setIsLoading(false)
		} else {
			setBlogs([])
		...
		<Drawer>
			...
			<DrawerContent>
				...
				<div className='container max-w-6xl mx-auto py-12'>
					<Input className='border-white' placeholder='Type to search blog...' onChange={debounceSearch} disabled={isLoading} />
